...
const Dashboard = () => {
    ...
    const { user, logout, roles, loading: authLoading, token, switchWorkspace } = useAuth();
    ...
    // ✅ Handle workspace selection — set context rồi mới navigate
    const handleSelectWorkspace = (tenantId) => {
        // Tìm workspace object trong danh sách đã fetch
        const ws = workspaces.find(w => w.id === tenantId);
        if (ws) {
            // Set workspace context → localStorage + AuthContext state
            switchWorkspace({
                id: ws.id,
                name: ws.name,
                role: ws.role,
                industryCode: ws.industryCode,
